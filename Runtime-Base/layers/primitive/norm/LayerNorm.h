...
class LayerNorm {
    ...
    void Initialization(){
        ...
        auto shift = Minitorch::HeUniformTensorInitialization(D, D); // {D}
...
public:
    ...
    PTR_T forward(const PTR_T &x){
        ...
        auto out = x->Copy();
        for (int i = 0; i < C * B; ++i) {
            auto x_c_mean = Minitorch::Mean(x, D, i * D);

            auto x_c_std = Minitorch::Std(x, D, i * D);
            x_c_std = Minitorch::AddElement(x_c_std, epsilon);

            for (int d = 0; d < D; ++d) {
                auto normalized = Minitorch::DivElement(Minitorch::SubElement(ref[i * D + d], x_c_mean), x_c_std); // ((x - mean) / (Std + epsilon)) = alpha
                // normalized = Minitorch::MulElement(normalized, scale_ref[d]); // alpha * scale
                // normalized = Minitorch::AddElement(normalized, shift_ref[d]); // alpha * scale + shift
